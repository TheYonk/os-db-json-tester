...
while current_time < time_to_run : 
   ...
   qry = qry + 1     
   for (imdb_id, title, imdb_rating, actor_character ) in cursor:
       if debug == 1 :
           print(" -> -> Movies for Actor: " + actor_name + " : Movie:  " + title);
   ...
   for (imdb_id, title, imdb_rating, director ) in cursor:
   ...
   if debug == 1 :
     print("Title search: " + str(search_title[0][0:6]))
   cursor.execute(find_movies_by_title, (search_title[0][0:6],))
   qry = qry + 1
